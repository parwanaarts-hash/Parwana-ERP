 ...
 *
 * All business logic lives in returnBillService.
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await getReturnBillByNumber(billNumber);

    if (result === null) {
      throw new ApiError(404, `Return Bill not found: billNumber=${billNumber}`, "ReturnBillNotFoundError");
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const results = await listReturnBills(query);
    res.json(results);
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await createReturnBill({
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await getReturnBill(id);

    if (result === null) {
      throw new ApiError(404, `Return Bill not found: id=${id}`, "ReturnBillNotFoundError");
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await updateReturnBill(id, {
      billDate:    body.billDate,
