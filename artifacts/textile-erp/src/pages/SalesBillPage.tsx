...
export default function SalesBillPage() {
  ...
  // ── Queries ───────────────────────────────────────────────────────────────
  const { data, isLoading, refetch } = useListSalesBills({
    limit: pageSize,
  ...
  const { data: fullRecord, isLoading: isLoadingRecord } = useGetSalesBill(
    ...
    {
      query: {
        queryKey: getGetSalesBillQueryKey(selectedId ?? 0),
  ...
  // id→name map for list view
  const partyMap: Record<number, string> = Object.fromEntries(
    (partiesData?.rows ?? []).map((p) => [p.id, p.name])
  );

  // Gate passes available for the form selector:
  // Add mode  → only unlinked (salesBillId == null)
  // Edit mode → unlinked + those already linked to this bill
  const allGatePasses = allGatePassesData?.rows ?? [];
  const availableGatePasses =
    mode === "edit" && selectedId
      ? allGatePasses.filter(
          (gp) => gp.salesBillId == null || gp.salesBillId === selectedId
        )
    ...
    if (mode === "add") {
      createBill.mutate({ data: formData }, {
        ...
        onError: (err: any) => {
          toast({
            title: "Error",
            description: err?.message ?? "Failed to create sales bill.",
            variant: "destructive",
          });
        },
      });
      ...
      updateBill.mutate({ id: selectedId, data: formData }, {
        ...
        onError: (err: any) => {
          toast({
            title: "Error",
            description: err?.message ?? "Failed to update sales bill.",
            variant: "destructive",
          });
        },
      });
  ...
  const handleDelete = () => {
    ...
    deleteBill.mutate({ id: selectedId }, {
      ...
      onError: (err: any) => {
        toast({
          title: "Error",
          description: err?.message ?? "Failed to delete sales bill.",
          variant: "destructive",
        });
        setIsDeleteDialogOpen(false);
      },
        ...
        <EntityTable
          columns={[
            ...
            },
            {
              ...
              label: "Bill Type",
              width: "90px",
              render: (row: SalesBill) => row.billType ?? "—",
            },
            {
              key: "cashPayment",
              label: "Cash Payment",
              width: "110px",
              render: (row: SalesBill) => row.cashPayment ?? "—",
            },
            {
              key: "bankPayment",
              label: "Bank Payment",
              width: "110px",
              render: (row: SalesBill) => row.bankPayment ?? "—",
            },
            {
              key: "remarks",
              label: "Remarks",
              render: (row: SalesBill) => row.remarks ?? "—",
            },
            {
              key: "gatePassIds",
          ...
          emptyMessage="No sales bills found. Press F2 to create the first one."
        />
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h3 className="font-semibold text-base">
                {mode === "add" ? "New Sales Bill" : "Edit Sales Bill"}
              </h3>
