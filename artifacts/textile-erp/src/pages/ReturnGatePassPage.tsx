...
export default function ReturnGatePassPage() {
  ...
  const { data, isLoading, refetch } = useListReturnGatePasses({ limit: pageSize, offset: page * pageSize });
  const { data: fullRecord, isLoading: isLoadingRecord } = useGetReturnGatePass(
    selectedId ?? 0,
    { query: { queryKey: getGetReturnGatePassQueryKey(selectedId ?? 0), enabled: mode === "edit" && selectedId !== null } }
  );
  const { data: nextNumber } = useNextDocumentNumber(DOC_TYPES.ReturnGatePass);
  ...
  const invalidateNextNumber = () =>
    queryClient.invalidateQueries({ queryKey: nextNumberQueryKey(DOC_TYPES.ReturnGatePass) });
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background" data-testid="page-return-gate-pass">
      ...
      <div className="flex-1 overflow-auto p-4 flex flex-col gap-4">
        ...
        <div className="max-h-44 overflow-hidden rounded-md">
          <EntityTable
            columns={[
              ...
              { key: "salePartyId", label: "Sale Party", render: (row: ReturnGatePass) => partyMap[row.salePartyId] ?? `#${row.salePartyId}` },
              { key: "items", label: "Items", width: "60px", render: (row: ReturnGatePass) => String(row.items?.length ?? 0) },
              { key: "remarks", label: "Remarks", render: (row: ReturnGatePass) => row.remarks ?? "—" },
              { key: "returnBillId", label: "Bill #", width: "80px", render: (row: ReturnGatePass) => row.returnBillId ? String(row.returnBillId) : "—" },
            ]}
            rows={data?.rows ?? []} total={data?.total ?? 0} isLoading={isLoading} selectedId={selectedId}
            onRowClick={(row) => startEdit(row.id)} emptyMessage="No return gate passes found."
          />
        </div>
        ...
        <div className="bg-card border rounded-md p-4 shadow-sm shrink-0" data-testid="container-form-section">
          <div className="flex items-center gap-3 mb-4">
            <h3 className="font-semibold text-base">{mode === "edit" ? "Edit Return Gate Pass" : "New Return Gate Pass"}</h3>
