...
import { ShikanjaForm, ShikanjaFormData } from "@/components/master/forms/ShikanjaForm";
import {
...
export default function ShikanjaPage() {
  ...
  const onSubmit = (formData: ShikanjaFormData) => {
    const payload = { name: formData.name };

    if (mode === "add") {
      createShikanja.mutate({ data: payload }, {
        onSuccess: () => {
      ...
      updateShikanja.mutate({ id: selectedId, data: payload }, {
        onSuccess: () => {
    ...
    <div className="flex flex-col h-full w-full bg-background" data-testid="page-shikanja">
      ...
      <div className="flex-1 overflow-auto p-4 flex flex-col gap-3">
        ...
        >
          ...
          />
          <TBtn
            icon={Trash2} label={deleteShikanja.isPending ? "Deleting…" : "Delete"}
            onClick={() => setIsDeleteDialogOpen(true)}
            disabled={!selectedId || mode !== "edit" || deleteShikanja.isPending}
        ...
        <div className="bg-card border rounded-md p-4 shadow-sm shrink-0">
          <h3 className="font-semibold text-base mb-3">
            {mode === "edit" ? "Edit Shikanja" : "New Shikanja"}
          </h3>
          <ShikanjaForm
            key={formKey}
          ...
          <Table>
            <TableHeader className="bg-muted/50 sticky top-0 z-10 shadow-sm">
              <TableRow>
                ...
                <TableHead className="font-medium text-foreground">Shikanja Name</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
