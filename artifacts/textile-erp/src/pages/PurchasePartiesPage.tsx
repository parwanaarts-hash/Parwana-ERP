...
import {
  ...
  getGetPurchasePartyQueryOptions,
...
export default function PurchasePartiesPage() {
  ...
  const [lookupRecord, setLookupRecord] = useState<PurchaseParty | null>(null);
  ...
  const handleSerialCommit = async (val: string) => {
    ...
    // If already editing this record, do nothing
    if (mode === "edit" && selectedId === id) return;
    try {
      const record = await queryClient.fetchQuery(getGetPurchasePartyQueryOptions(id));
      setLookupRecord(record as PurchaseParty);
      startEdit((record as PurchaseParty).id);
      setFormKey((k) => k + 1);
    } catch {
      toast({
        title: "Record Not Found",
        description: `Purchase Party #${id} does not exist. Form is ready for a new entry.`,
        variant: "destructive",
      });
    }
  };
