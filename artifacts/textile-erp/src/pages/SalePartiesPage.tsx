...
export default function SalePartiesPage() {
  ...
  const handleRefresh = () => { queryClient.invalidateQueries({ queryKey: getListSalePartiesQueryKey() }); refetch(); exitForm(); setSearchInput(""); setSearch(""); };
  ...
  const onSubmit = (formData: SalePartyInput) => {
    if (mode === 'add') {
      createSaleParty.mutate({ data: formData }, {
        onSuccess: () => { toast({ title: "Success", description: "Sale Party created." }); queryClient.invalidateQueries({ queryKey: getListSalePartiesQueryKey() }); exitForm(); },
      ...
      updateSaleParty.mutate({ id: selectedId, data: formData }, {
        onSuccess: () => { toast({ title: "Success", description: "Sale Party updated." }); queryClient.invalidateQueries({ queryKey: getListSalePartiesQueryKey() }); exitForm(); },
        onError: (err: any) => toast({ title: "Error", description: err.message || "Failed.", variant: "destructive" })
      });
    }
  };
  ...
  const handleDelete = () => {
    ...
    deleteSaleParty.mutate({ id: selectedId }, {
      onSuccess: () => { toast({ title: "Success", description: "Sale Party deleted." }); queryClient.invalidateQueries({ queryKey: getListSalePartiesQueryKey() }); setIsDeleteDialogOpen(false); exitForm(); },
    ...
    <div className="flex flex-col h-full w-full bg-background" data-testid="page-sale-parties">
      ...
      <div className="flex-1 overflow-auto p-4 flex flex-col gap-4 relative">
        ...
        <Toolbar 
          ...
          isSaving={createSaleParty.isPending || updateSaleParty.isPending} isDeleting={deleteSaleParty.isPending}
        />
        ...
        {(mode === 'add' || mode === 'edit') && (
          <div className="bg-card border rounded-md p-4 shadow-sm shrink-0">
            <h3 className="font-semibold text-lg mb-4">{mode === 'add' ? 'Add Sale Party' : 'Edit Sale Party'}</h3>
            <SalePartyForm
